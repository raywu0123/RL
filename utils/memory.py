...
class ReplayMemory:
    ...
    def __init__(self, args, capacity, device, num_ales=None):
        ...
        # Initial importance sampling weight β, annealed to 1 over
        ...
        self.gammas = self.discount ** torch.FloatTensor(range(self.n)).to(
            self.device
        ).unsqueeze(0)
        self.frame_offsets = torch.IntTensor(range(-(self.history - 1), 1)).to(
            device=self.device
        ).unsqueeze(0)
    ...
    def update_sections(self, batch_size):
        ...
        if self.section_size.size(0) != capacity:
            ...
            pdf = torch.FloatTensor(1.0 / np.arange(1, capacity + 1)).to(
                device=self.device
            ...
            self.section_offset = torch.cat((
                self.section_offset,
                torch.LongTensor([0]).to(device=self.device)
            ))
    ...
    def reset(self, observations):
        self.observations[:, self.history - 1] = observations.mul(255.0).to(
            device=self.device,
            dtype=torch.uint8,
        )
    ...
    def append(self, observations, actions, rewards, terminals):
        ...
        self.rewards[terminal_mask, prev_offset - (self.history - 1)] = \
            rewards[terminal_mask].float()
        self.observations[:, curr_offset] = \
            observations.mul(255.0).to(device=self.device, dtype=torch.uint8)
        self.frame_number[:, curr_offset] = \
            nonterminal.int() * (self.frame_number[:, prev_offset] + 1)

        if self.priority_replay:
            self.priority_view[:, self.index] = \
                self.priority.max() + float(np.finfo(np.float32).eps)
            self.rank = self.priority.sort(descending=True)[1]

        if (self.epoch > 0) and (self.index == 0):
            self.observations[:, self.history - 1] = self.observations[
                :, self.steps_per_ale + self.history - 1
            ]
            self.frame_number[:, self.history - 1] = self.frame_number[
                :, self.steps_per_ale + self.history - 1
            ]
    ...
    def sample(self, batch_size=0, indices=None):
        ...
        if self.priority_replay:
            ...
            weights = (capacity * P) ** -self.priority_weight
        ...
        base_frame_numbers = self.frame_number[
            ale_indices, step_indices + self.history - 1
        ].unsqueeze(-1).expand(-1, self.history)
        ...
        states_mask = (
            actual_frame_numbers == expected_frame_numbers
        ...
        next_states_mask = (
            next_actual_frame_numbers == (expected_frame_numbers + self.n)
        ).float().unsqueeze(-1).unsqueeze(-1)
        next_states = self.states_view[
            ale_indices,
            step_indices + self.n
        ].float().div_(255.0) * next_states_mask
